...
describe("modelDirUtils cache policy (#1279, #1399)", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.OPENWHISPR_CHANNEL;
  ...
  it("keeps the custom macOS cache isolated from the official app", () => {
    setPlatform("darwin");
    const home = path.join(tempRoot, "Users", "stan");
    ...
    assert.strictEqual(getCacheRoot(), path.join(home, ".cache", "openwhispr-custom"));
  });

  it("keeps the custom XDG cache isolated from the official app", () => {
    setPlatform("linux");
    const home = path.join(tempRoot, "home", "stan");
    const xdgCacheHome = path.join(tempRoot, "xdg-cache");
    process.env.OPENWHISPR_CHANNEL = "custom";
    ...
    assert.strictEqual(getCacheRoot(), path.join(xdgCacheHome, "openwhispr-custom"));
  });
  ...
  it("uses a separate ASCII-safe Windows fallback for the custom app", () => {
    setPlatform("win32");
    const home = path.join(tempRoot, "Users", "stan");
    const programData = path.join(tempRoot, "ProgramData");
    process.env.OPENWHISPR_CHANNEL = "custom";
    ...
    assert.strictEqual(getCacheRoot(), path.join(programData, "OpenWhisprCustom", "cache"));
  });
