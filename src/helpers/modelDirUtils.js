...
function getAsciiSafeFallbackRoot(cacheNamespace) {
  const applicationDir = cacheNamespace === "openwhispr-custom" ? "OpenWhisprCustom" : "OpenWhispr";
  const candidates = [
    path.join(process.env.ProgramData || "C:\\ProgramData", applicationDir, "cache"),
    path.join(process.env.SystemDrive || "C:", applicationDir, "cache"),
  ];
...
function getPreferredCacheRoot(homeCache, cacheNamespace) {
  ...
  if (process.platform === "win32") {
    ...
    if (redirectedProfile && path.isAbsolute(redirectedProfile)) {
      return path.join(redirectedProfile, ".cache", cacheNamespace);
    }
  }
  ...
  if (process.platform === "linux") {
    ...
    if (xdgCacheHome && path.isAbsolute(xdgCacheHome)) {
      return path.join(xdgCacheHome, cacheNamespace);
    }
  }
...
function getCacheRoot() {
  ...
  const homeCache = path.join(homeDir, ".cache", cacheNamespace);
  let targetRoot = getPreferredCacheRoot(homeCache, cacheNamespace);

  if (process.platform === "win32" && pathHasProblematicChars(targetRoot)) {
    targetRoot = getAsciiSafeFallbackRoot(cacheNamespace) || homeCache;
  }
